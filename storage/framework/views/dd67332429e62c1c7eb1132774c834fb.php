<?php $__env->startSection('title', 'Surat Masuk | SIMA'); ?>

<?php $__env->startSection('content'); ?>
<header class="mb-8 flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-end reveal">
    <div>
        <h1 class="font-serif text-4xl font-medium tracking-tight">Surat Masuk</h1>
        <p class="text-sm text-muted mt-2">Register surat masuk sesuai format buku agenda.</p>
    </div>
    <button onclick="document.getElementById('modalTambah').showModal()" class="btn-primary text-sm self-start sm:self-auto">
        + Tambah Surat Masuk
    </button>
</header>

<?php if(session('success')): ?>
    <div class="mb-6 p-3 rounded-md bg-paleGreen text-inkGreen text-sm border border-[#D7E5D5] reveal">
        <?php echo e(session('success')); ?>

    </div>
<?php endif; ?>

<div class="mb-6 flex justify-between items-center reveal" style="transition-delay: 50ms;">
    <form action="<?php echo e(route('surat-masuk')); ?>" method="GET" class="relative w-full max-w-xl">
        <i class="ph ph-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-muted text-lg"></i>
        <input
            type="text"
            name="search"
            value="<?php echo e(request('search')); ?>"
            placeholder="Cari no. urut, nomor surat, pengirim, perihal, atau keterangan..."
            class="w-full pl-10 pr-10 py-2.5 text-sm bg-surface border border-borderline rounded-md focus:border-ink focus:outline-none transition-colors"
            autocomplete="off"
        >
        <?php if(request('search')): ?>
            <a href="<?php echo e(route('surat-masuk')); ?>" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-inkRed transition-colors" title="Hapus pencarian">
                <i class="ph ph-x"></i>
            </a>
        <?php endif; ?>
    </form>
</div>

<section class="bento-card !p-0 overflow-hidden reveal" style="transition-delay: 100ms;">
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm min-w-[1380px] register-table">
            <thead class="bg-canvas text-muted">
                <tr class="border-b border-borderline">
                    <th rowspan="2" class="px-4 py-3 font-medium text-center align-middle w-24">NO.<br>URUT</th>
                    <th colspan="2" class="px-4 py-3 font-medium text-center border-l border-borderline">SURAT MASUK</th>
                    <th rowspan="2" class="px-4 py-3 font-medium text-center align-middle min-w-[280px] border-l border-borderline">NAMA, PEKERJAAN DAN<br>ALAMAT PENGIRIM</th>
                    <th rowspan="2" class="px-4 py-3 font-medium text-center align-middle min-w-[320px] border-l border-borderline">PERIHAL / URAIAN<br>POKOK MASALAH</th>
                    <th rowspan="2" class="px-4 py-3 font-medium text-center align-middle min-w-[220px] border-l border-borderline">KETERANGAN</th>
                    <th rowspan="2" class="px-4 py-3 font-medium text-center align-middle w-40 border-l border-borderline">DOKUMEN</th>
                </tr>
                <tr class="border-b border-borderline">
                    <th class="px-4 py-3 font-medium text-center w-36 border-l border-borderline">TANGGAL</th>
                    <th class="px-4 py-3 font-medium text-center min-w-[230px] border-l border-borderline">NOMOR</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-borderline bg-surface">
                <?php $__empty_1 = true; $__currentLoopData = $surat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                        $extension = $s->file_pdf ? strtolower(pathinfo($s->file_pdf, PATHINFO_EXTENSION)) : null;
                        $isImage = in_array($extension, ['jpg', 'jpeg', 'png', 'webp', 'gif']);
                        $fileUrl = $s->file_pdf ? route('surat-masuk.file', $s) : null;
                    ?>
                    <tr class="hover:bg-canvas/60 transition-colors align-top">
                        <td class="px-4 py-4 text-center font-mono text-xs"><?php echo e($s->nomor_urut ?: $s->id); ?></td>
                        <td class="px-4 py-4 text-center whitespace-nowrap border-l border-borderline"><?php echo e(\Carbon\Carbon::parse($s->tanggal_surat)->format('d-m-Y')); ?></td>
                        <td class="px-4 py-4 font-mono text-xs break-words border-l border-borderline"><?php echo e($s->nomor_surat); ?></td>
                        <td class="px-4 py-4 whitespace-pre-line break-words border-l border-borderline"><?php echo e($s->pengirim); ?></td>
                        <td class="px-4 py-4 whitespace-pre-line break-words border-l border-borderline"><?php echo e($s->perihal); ?></td>
                        <td class="px-4 py-4 whitespace-pre-line break-words text-muted border-l border-borderline"><?php echo e($s->keterangan ?: '-'); ?></td>
                        <td class="px-4 py-4 border-l border-borderline">
                            <?php if($s->file_pdf): ?>
                                <div class="flex flex-col items-start gap-2">
                                    <button
                                        type="button"
                                        data-url="<?php echo e($fileUrl); ?>"
                                        data-type="<?php echo e($isImage ? 'image' : 'pdf'); ?>"
                                        data-title="Surat Masuk <?php echo e($s->nomor_surat); ?>"
                                        onclick="openDocumentPreview(this.dataset.url, this.dataset.type, this.dataset.title)"
                                        class="inline-flex items-center gap-2 text-ink font-medium hover:underline"
                                    >
                                        <i class="ph-fill <?php echo e($isImage ? 'ph-file-image text-inkBlue' : 'ph-file-pdf text-inkRed'); ?> text-lg"></i>
                                        Lihat
                                    </button>
                                    <a href="<?php echo e($fileUrl); ?>?download=1" class="text-xs text-muted hover:text-ink hover:underline">Unduh</a>
                                </div>
                            <?php else: ?>
                                <span class="text-muted text-xs italic">Tidak ada file</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="7" class="px-6 py-14 text-center text-muted">Belum ada data surat masuk.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
</section>

<?php echo $__env->make('surat.modals.tambah_masuk', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php echo $__env->make('surat.modals.preview_dokumen', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('components.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/surat/masuk.blade.php ENDPATH**/ ?>