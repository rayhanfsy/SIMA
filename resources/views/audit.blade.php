...
@section('title', 'Audit Keamanan | SIMA')
...
<div class="reveal">
    ...
    <section class="bg-surface border border-borderline rounded-xl shadow-sm">
        <form method="GET" action="{{ route('audit') }}" class="p-4 border-b border-borderline">
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-3 items-center">
                ...
                <div class="xl:col-span-2 flex gap-2 min-w-0">
                    ...
                    @if(request()->hasAny(['q', 'event', 'date_from', 'date_to']))
                        <a href="{{ route('audit') }}" class="shrink-0 px-3 py-2.5 text-sm text-muted hover:text-ink transition-colors" title="Reset filter">
                            Reset
                        </a>
                    @endif
                </div>
                ...
                <tbody class="divide-y divide-borderline">
                    @forelse($logs as $log)
                        <tr class="hover:bg-canvas/70 transition-colors align-top">
                            ...
                            <td class="px-4 py-3.5 font-mono text-[10px] leading-4 text-[#555451] break-all" title="{{ $log->ip_address ?: '-' }}">
                                {{ $log->ip_address ?: '-' }}
                            </td>
                        </tr>
                    @empty
        ...
        @if($logs->hasPages())
            <div class="px-4 py-3.5 border-t border-borderline flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ $logs->previousPageUrl() }}" class="px-3 py-1.5 border border-borderline rounded-md hover:bg-canvas">Sebelumnya</a>
                    @endif

                    <span class="px-2 text-muted">{{ $logs->currentPage() }} / {{ $logs->lastPage() }}</span>

                    @if($logs->hasMorePages())
                        <a href="{{ $logs->nextPageUrl() }}" class="px-3 py-1.5 border border-borderline rounded-md hover:bg-canvas">Berikutnya</a>
                    @else
