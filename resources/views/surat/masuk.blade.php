...
@section('title', 'Surat Masuk | SIMA')
...
<div class="mb-6 flex justify-between items-center gap-4 reveal" style="transition-delay: 50ms;">
    <form action="{{ route('surat-masuk') }}" method="GET" class="relative w-full max-w-xl">
        ...
        >
        @if(request('search'))
            <a href="{{ route('surat-masuk') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-inkRed transition-colors" title="Hapus pencarian">
                <i class="ph ph-x"></i>
            </a>
        @endif
    </form>
    <a href="{{ route('surat-masuk.export', request()->only('search')) }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm border border-borderline rounded-md hover:bg-canvas transition-colors shrink-0">
            ...
            <tbody class="divide-y divide-borderline bg-surface">
                @forelse($surat as $s)
                    ...
                    <tr class="hover:bg-canvas/60 transition-colors align-top">
                        ...
                        <td class="px-4 py-4 whitespace-pre-line break-words border-l border-borderline">{{ $s->pengirim }}</td>
                            ...
                            @if($s->file_pdf)
                                <div class="flex flex-col items-start gap-2">
                                    <button
                                        ...
                                        data-title="Surat Masuk {{ $s->nomor_surat }}"
