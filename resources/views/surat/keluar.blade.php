...
<div class="mb-6 flex justify-between items-center reveal" style="transition-delay: 50ms;">
    ...
    <a href="{{ route('surat-keluar.export', request()->only('search')) }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm border border-borderline rounded-md hover:bg-canvas transition-colors shrink-0">
                        ...
                        <td class="px-4 py-4 text-center border-l border-borderline">
                            @if(auth()->user()->hasRole('staf', 'admin'))
                                <div class="flex items-center justify-center gap-3">
                                    <button
                                        type="button"
                                        onclick="openEditKeluar({{ \Illuminate\Support\Js::from($s->id) }}, {{ \Illuminate\Support\Js::from(\Carbon\Carbon::parse($s->tanggal_surat)->format('Y-m-d')) }}, {{ \Illuminate\Support\Js::from($s->nomor_surat) }}, {{ \Illuminate\Support\Js::from($s->tujuan) }}, {{ \Illuminate\Support\Js::from($s->perihal) }}, {{ \Illuminate\Support\Js::from($s->keterangan) }})"
