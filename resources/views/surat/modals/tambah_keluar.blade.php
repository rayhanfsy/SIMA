<dialog id="modalKeluar" class="p-0 rounded-xl border border-borderline bg-surface w-[min(820px,94vw)] max-w-none shadow-xl">
    <div class="p-6 sm:p-8 max-h-[92vh] overflow-y-auto">
        ...
        <form id="formKeluar" action="{{ route('surat-keluar.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5">
            @csrf
            <input type="hidden" name="_method" id="keluarMethod" value="{{ old('_method') }}">
            <input type="hidden" name="_id" id="keluarId" value="{{ old('_id') }}">
            ...
            <div class="rounded-lg border border-borderline overflow-hidden">
                ...
                <div class="p-4 grid grid-cols-1 sm:grid-cols-12 gap-4">
                    <div class="sm:col-span-2 flex flex-col gap-1.5">
                        ...
                        <input type="text" name="nomor_urut" id="keluarNomorUrut" class="input-base" placeholder="Contoh: 155" value="{{ old('nomor_urut') }}" required>
                    </div>
                    <div class="sm:col-span-4 flex flex-col gap-1.5">
                        ...
                        <input type="date" name="tanggal_surat" id="keluarTanggal" class="input-base" value="{{ old('tanggal_surat') }}" required>
                    </div>
                    <div class="sm:col-span-6 flex flex-col gap-1.5">
                        ...
                        <input type="text" name="nomor_surat" id="keluarNomorSurat" class="input-base" placeholder="Nomor surat keluar" value="{{ old('nomor_surat') }}" required>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <textarea name="tujuan" id="keluarTujuan" rows="3" class="input-base resize-y" placeholder="Nama instansi, pihak, atau alamat tujuan" required>{{ old('tujuan') }}</textarea>
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-muted uppercase tracking-[0.05em]">Perihal / Uraian Pokok Masalah</label>
                <textarea name="perihal" id="keluarPerihal" rows="3" class="input-base resize-y" placeholder="Tuliskan perihal atau uraian pokok masalah" required>{{ old('perihal') }}</textarea>
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-muted uppercase tracking-[0.05em]">Keterangan</label>
                <textarea name="keterangan" id="keluarKeterangan" rows="2" class="input-base resize-y" placeholder="Contoh: periode, petugas, tindak lanjut, atau catatan lain">{{ old('keterangan') }}</textarea>
            </div>
...
<script>
    ...
    function openTambahKeluar() {
        ...
        document.getElementById('formKeluar').action = "{{ route('surat-keluar.store') }}";
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        @if(old('_id'))
            document.getElementById('formKeluar').action = '/surat-keluar/' + {{ \Illuminate\Support\Js::from(old('_id')) }};
            document.getElementById('keluarMethod').value = 'PUT';
            document.getElementById('keluarModalTitle').textContent = 'Edit Surat Keluar';
            document.getElementById('keluarSubmitBtn').textContent = 'Perbarui Surat Keluar';
        @endif
