<dialog id="modalTambah" class="p-0 rounded-xl border border-borderline bg-surface w-[min(820px,94vw)] max-w-none shadow-xl">
    <div class="p-6 sm:p-8 max-h-[92vh] overflow-y-auto">
        <div class="mb-6">
            <h3 class="font-serif text-2xl">Tambah Surat Masuk</h3>
        ...
        <form action="{{ route('surat-masuk.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5">
            @csrf
            ...
            <div class="rounded-lg border border-borderline overflow-hidden">
                ...
                <div class="p-4 grid grid-cols-1 sm:grid-cols-12 gap-4">
                    <div class="sm:col-span-2 flex flex-col gap-1.5">
                        ...
                        <input type="text" name="nomor_urut" class="input-base" placeholder="Contoh: 38" value="{{ old('nomor_urut') }}" required>
                    </div>
                    <div class="sm:col-span-4 flex flex-col gap-1.5">
                        ...
                        <input type="date" name="tanggal_surat" class="input-base" value="{{ old('tanggal_surat') }}" required>
                    </div>
                    <div class="sm:col-span-6 flex flex-col gap-1.5">
                        ...
                        <input type="text" name="nomor_surat" class="input-base" placeholder="Nomor surat masuk" value="{{ old('nomor_surat') }}" required>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <textarea name="pengirim" rows="3" class="input-base resize-y" placeholder="Instansi, alamat/identitas pengirim" required>{{ old('pengirim') }}</textarea>
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-muted uppercase tracking-[0.05em]">Perihal / Uraian Pokok Masalah</label>
                <textarea name="perihal" rows="3" class="input-base resize-y" placeholder="Tuliskan perihal atau uraian pokok masalah" required>{{ old('perihal') }}</textarea>
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-muted uppercase tracking-[0.05em]">Keterangan</label>
                <textarea name="keterangan" rows="2" class="input-base resize-y" placeholder="Contoh: jadwal kegiatan, tindak lanjut, atau catatan lain">{{ old('keterangan') }}</textarea>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" class="btn-primary">Simpan Surat Masuk</button>
            </div>
        </form>
    </div>
</dialog>
